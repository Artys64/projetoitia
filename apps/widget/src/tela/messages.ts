...
export function renderMessages(region:HTMLElement,client?:ChatClient):()=>void {
  ...
  function append(messages:ChatMessage[]){for(const m of messages){sequence=Math.max(sequence,m.sequence);if(rendered.has(m.id))continue;rendered.add(m.id);const article=document.createElement('article');article.className=`chat-message ${m.role}`;const author=document.createElement('strong');author.textContent=m.role==='user'?'Você':'Nora';const text=document.createElement('p');text.textContent=m.content;article.append(author,text);if(m.sources.length){const sources=document.createElement('div');sources.className='chat-sources';const label=document.createElement('span');label.textContent='Artigos consultados';sources.append(label);for(const source of m.sources){const button=document.createElement('button');button.type='button';button.textContent=source.title;button.addEventListener('click',()=>void (async()=>{button.disabled=true;try{const current=await api.source(source.articleId,source.version);const detail=document.createElement('p');detail.className='source-detail';detail.textContent=current.status==='updated'?'Este artigo foi atualizado desde a resposta.':current.content;button.after(detail)}catch{showError(new Error('Não foi possível abrir este artigo.'))}finally{button.disabled=false}})());sources.append(button)}article.append(sources)}history.append(article)}}
