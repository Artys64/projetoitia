...
test('painel administra texto e Markdown sem misturar empresas ou rascunhos', async t => {
  ...
  const app = buildApp({ database: fixture.db, logger: false, adminOrigin: origin })
  t.after(() => app.close())
  ...
  assert.equal((await createPostgresKnowledgeSearch(fixture.db)('trocas', 'company_a')).length, 0)
  ...
  assert.match((await createPostgresKnowledgeSearch(fixture.db)('30 dias', 'company_a')).map(hit => hit.text).join('\n'), /30 dias/)
  assert.equal((await createPostgresKnowledgeSearch(fixture.db)('trocas', 'company_b')).length, 0)
  ...
  assert.match((await createPostgresKnowledgeSearch(fixture.db)('45 dias', 'company_a')).map(hit => hit.text).join('\n'), /45 dias/)
