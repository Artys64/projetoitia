...
export class Database {
  ...
  async transaction<T>(tenant: string | null, work: (sql: Sql) => Promise<T>): Promise<T> {
    ...
    try {
      ...
      await sql.query("SELECT set_config('app.tenant_id',$1,true), set_config('app.token_hash','',true), set_config('TimeZone','UTC',true), set_config('statement_timeout','10000',true), set_config('lock_timeout','5000',true)", [tenant ?? ''])
