...
import { importArticles,importInstallation,issueAdminSession,provisionRuntime } from './admin.js'
  ...
  switch(command) {
    ...
    case 'metrics': {
      ...
      for(const t of tenants) console.info(JSON.stringify(await db.transaction(t.id,async sql=>{
        return {tenantId:t.id,...await readTenantMetrics(sql)}
      })))
      break
    }
    case 'prune-rate-buckets': await db.pool.query('DELETE FROM session_rate_buckets WHERE expires_at<now()');break
    default: throw new Error('Use: db migrate|provision|installation arquivo.json|articles arquivo.json|admin-session empresa|metrics|prune-rate-buckets')
  }
